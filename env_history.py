...
def _get_base_query(base_query: str, start_info: str, memory: List[str]) -> str:
    ...
    if len(memory) > 0:
        #query += '\n\nYour causal inferences about your last failed attempt are summarized as follows:'
        #query += '\n\nYour causal reasoning findings for the task below:'
        query += '\n\nYour memory for the task below:'
        #query += '\n\nBased on your past failures, you have reflected on the following reasons and should take care to learn the following lessons before proceeding with the following tasks.'
        #query += '\n\nYour attributed memory for the task below:'
