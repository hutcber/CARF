...
from generate_reflections import update_memory, qwen_update_counterfactual_imaginary, router_update_counterfactual_imaginary, update_counterfactual_imaginary, router_update_memory
...
def main(args) -> None:
    ...
    while trial_idx < args.num_trials:
        ...
        if args.use_memory and trial_idx < args.num_trials-1:
            if args.model.startswith('openai'):
                env_configs: List[Dict[str, Any]] = router_update_counterfactual_imaginary(trial_log_path, env_configs)#_update_counterfactual_imaginary(trial_log_path, env_configs)#update_memory(trial_log_path, env_configs)#
            elif args.model.startswith('qwen'):
                env_configs: List[Dict[str, Any]] = qwen_update_counterfactual_imaginary(trial_log_path, env_configs)
            else:
                env_configs: List[Dict[str, Any]] = update_memory(trial_log_path, env_configs)#update_counterfactual_imaginary(trial_log_path, env_configs)
            #env_configs: List[Dict[str, Any]] = update_counterfactual_imaginary(trial_log_path, env_configs)
